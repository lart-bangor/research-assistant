...
class AgtTaskTraitRating(BaseModel):
    ...
    trait: constr(strip_whitespace=True, to_lower=True, regex=p.SHORT_TEXT) = Field(
        description="Name of the trait being rated."
    )

    rating: confloat(ge=0, le=100) = Field(description="Rating value for the trait.")
...
class AgtTaskTrialRatings(BaseModel):
    ...
    trial: constr(
        strip_whitespace=True, to_lower=True, regex=agt_trial_pattern
    ) = Field(description="Name of the trial (audio stimulus) being rated.")

    ratings: conlist(item_type=AgtTaskTraitRating, unique_items=True) = Field(
...
class AgtTaskResponse(ResponseBase):
    ...
    stimulus_ratings: conlist(item_type=AgtTaskTrialRatings, unique_items=True) = Field(
