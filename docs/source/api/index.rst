.. _api_documentation:

API Documentation
=================

Backend API (Python)
^^^^^^^^^^^^^^^^^^^^

.. currentmodule:: research_assistant

.. autosummary::
    :toctree: generated
    :recursive:
    :nosignatures:

    agt
    app
    atolc
    booteel
    conclusion
    config
    consent
    datavalidator
    lsbq
    memorygame
    settings
    utils


Frontend API (JavaScript)
^^^^^^^^^^^^^^^^^^^^^^^^^

.. caution::
  The included JavaScript API documentation is generated semi-automatically with
  `sphinx-js <https://github.com/mozilla/sphinx-js>`_, which unfortunely doesn't support
  the full set of `jsdoc <https://jsdoc.app/>`_ directives. It can therefore
  lack detail and/or be out of sync. If in doubt we recommend running :code:`jsdoc` on
  the individual JavaScript files yourself to generate more detailed API documentation.

.. toctree::
    :includehidden:
    :hidden:

    frontend/lart.js/lart
    frontend/booteel.js/booteel

+---------------------------------------------------+------------------------------+
| :doc:`lart.js <frontend/lart.js/lart>`            | The L'ART JavaScript Library |
+---------------------------------------------------+------------------------------+
| :doc:`booteel.js <frontend/booteel.js/booteel>`   | The Booteel Frontend Library |
+---------------------------------------------------+------------------------------+
